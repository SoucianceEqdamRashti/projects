...
namespace BookingApi.Tests.DataAccess.Services
{
    public class FakeBookingRespository : IDataAccessRepository
    {
        ...
        public Task<IEnumerable<VehicleModel>> GetVehicleByRegistrationNumberAsync(string registrationNumber)
        {
            if (registrationNumber.Equals("DOESNOTEXIST"))
                return Task.FromResult(Enumerable.Empty<VehicleModel>());
            ...
            IEnumerable<VehicleModel> ListOfVehiclesFromRepository = new List<VehicleModel> { vehicle };
        ...
        public Task UpdateBookingAndVehicleAsync(RentalModel booking, VehicleModel vehicle)
        {
            return Task.FromResult(1);
        }
    }
