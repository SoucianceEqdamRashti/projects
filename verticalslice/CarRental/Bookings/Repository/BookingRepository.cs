...
namespace BookingApi.Bookings.Repository
{
    public class BookingRepository : IDataAccessRepository
    {
        ...
        public async Task<IEnumerable<GetAllVehiclesCommandResult>> GetVehicleByRegistrationNumberAsync(string registrationNumber)
        {
            ...
            using var connection = _dbConnectionFactory.CreateConnection();
            var vehicleFound = await connection.QueryAsync<GetAllVehiclesCommandResult>(query, param:parameters);
            if (vehicleFound == null)
            {
                return Enumerable.Empty<GetAllVehiclesCommandResult>();
            }
            return vehicleFound;
        }
        ...
        public async Task UpdateBookingAndVehicleAsync(RentalModel booking, GetAllVehiclesCommandResult vehicle)
        {
            var bookingNumber = booking.BookingNumber;
        ...
        public async Task<IEnumerable<GetAllVehiclesCommandResult>> GetAllVehiclesAsync()
        {          
            _logger.LogInformation("Getting all vehicles");
            const string query = @"SELECT * FROM dbo.Vehicles";
            using var connection = _dbConnectionFactory.CreateConnection();
            var vehicleFound = await connection.QueryAsync<GetAllVehiclesCommandResult>(query);
            if (vehicleFound == null)
            {
                return Enumerable.Empty<GetAllVehiclesCommandResult>();
            }
        ...
        public async Task<IEnumerable<RentalModel>> GetAllBookingsAsync()
        {
            _logger.LogInformation("Getting all bookings");
            const string query = @"SELECT * FROM dbo.Bookings";
            using var connection = _dbConnectionFactory.CreateConnection();
            var bookingsFound = await connection.QueryAsync<RentalModel>(query);
